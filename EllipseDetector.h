...
public:
	...
	// constructors
	EllipseDetector(const vector< vector<Point> > &contours, const vector<Vec4i> &hierarchy) : contours(contours), hierarchy(hierarchy)
	{
		...
		for (int i = 0; i != contours.size(); i++)
		{
			if (hierarchy[i][Hierarchy::Parent] == -1)
			{
				...
				if (rect.width > 30 && rect.height > 30)
				{
					...
					//Does the plate match and the main axes are similar?
